...
export namespace Type
{
    ...
    export type NamedColor = "black" | "silver" | "gray" | "white" | "maroon" | "red" | "purple" | "fuchsia" | "green" | "lime" | "olive" | "yellow" | "navy" |
        "blue" | "teal" | "aqua" | "aliceblue" | "antiquewhite" | "aquamarine" | "azure" | "beige" | "bisque" | "blanchedalmond" | "blueviolet" | "brown" |
        ...
        "dimgray" | "dimgrey" | "dodgerblue" | "firebrick" | "floralwhite" | "forestgreen" | "gainsboro" | "ghostwhite" | "gold" | "goldenrod" | "greenyellow"
        ...
        "mediumvioletred" | "midnightblue" | "mintcream" | "mistyrose" | "moccasin" | "navajowhite" | "oldlace" | "olivedrab" | "orange" | "orangered" |
        "orchid" | "palegoldenrod" | "palegreen" | "paleturquoise" | "palevioletred" | "papayawhip" | "peachpuff" | "peru" | "pink" | "plum" | "powderblue" |
        "rebeccapurple" | "rosybrown" | "royalblue" | "saddlebrown" | "salmon" | "sandybrown" | "seagreen" | "seashell" | "sienna" | "skyblue" | "slateblue" |
        "slategray" | "slategrey" | "snow" | "springgreen" | "steelblue" | "tan" | "thistle" | "tomato" | "transparent" | "turquoise" | "violet" | "wheat" |
    ...
    export const isFlounderType: EvilType.Validator.IsType<FlounderType> = EvilType.Validator.isOr(EvilType.Validator.isEnum([ "trispot", "tetraspot" ] as
        const), EvilType.Validator.isEnum([ "stripe", "diline", "triline" ] as const));
    export const isLayoutAngle: EvilType.Validator.IsType<LayoutAngle> = EvilType.Validator.isEnum([ "regular", "alternative" ] as const);
    export const isHexColor: EvilType.Validator.IsType<HexColor> = EvilType.Validator.isDetailedString({ pattern:"^#(?:[0-9A-Fa-f]){3,4,6,8}$", }, "u");
    export const isNamedColor: EvilType.Validator.IsType<NamedColor> = EvilType.Validator.isEnum([ "black", "silver", "gray", "white", "maroon", "red",
        ...
        "snow", "springgreen", "steelblue", "tan", "thistle", "tomato", "transparent", "turquoise", "violet", "wheat", "whitesmoke", "yellowgreen" ] as const);
    ...
    export const isNamedDirectionAngle: EvilType.Validator.IsType<NamedDirectionAngle> = EvilType.Validator.isEnum([ "right", "right-down", "down",
        "left-down", "left", "left-up", "up", "right-up" ] as const);
    export const isDirectionAngle: EvilType.Validator.IsType<DirectionAngle> = EvilType.lazy(() => EvilType.Validator.isOr(isNamedDirectionAngle, isSignedRate)
        );
    export const isArgumentsBase = EvilType.lazy(() => EvilType.Validator.isSpecificObject(argumentsBaseValidatorObject, { additionalProperties: false }));
    export const isSpotArguments = EvilType.lazy(() => EvilType.Validator.isSpecificObject(spotArgumentsValidatorObject, { additionalProperties: false }));
    export const isLineArguments = EvilType.lazy(() => EvilType.Validator.isSpecificObject(lineArgumentsValidatorObject, { additionalProperties: false }));
    export const isArguments: EvilType.Validator.IsType<Arguments> = EvilType.lazy(() => EvilType.Validator.isOr(isSpotArguments, isLineArguments));
    export const argumentsBaseValidatorObject: EvilType.Validator.ObjectValidator<ArgumentsBase> = ({ $schema: EvilType.Validator.isOptional(
        EvilType.Validator.isJust("https://raw.githubusercontent.com/wraith13/flounder.style.js/master/generated/schema.json#" as const)), type: isFlounderType
        , layoutAngle: EvilType.Validator.isOptional(EvilType.Validator.isOr(isLayoutAngle, isSignedRate)), offsetX: EvilType.Validator.isOptional(
        isSignedPixel), offsetY: EvilType.Validator.isOptional(isSignedPixel), foregroundColor: isColor, backgroundColor: EvilType.Validator.isOptional(isColor
        ), intervalSize: EvilType.Validator.isOptional(isPixel), depth: isRate, blur: EvilType.Validator.isOptional(isPixel), maxPatternSize:
        EvilType.Validator.isOptional(isPixel), reverseRate: EvilType.Validator.isOptional(EvilType.Validator.isOr(isSignedRate, EvilType.Validator.isJust(
        "auto" as const), EvilType.Validator.isJust("-auto" as const))), anglePerDepth: EvilType.Validator.isOptional(EvilType.Validator.isOr(isSignedRate,
        EvilType.Validator.isJust("auto" as const), EvilType.Validator.isJust("-auto" as const))), maximumFractionDigits: EvilType.Validator.isOptional(isCount
        ), });
    export const spotArgumentsValidatorObject: EvilType.Validator.ObjectValidator<SpotArguments> = EvilType.Validator.mergeObjectValidator(
        argumentsBaseValidatorObject, { type: EvilType.Validator.isEnum([ "trispot", "tetraspot" ] as const), layoutAngle: EvilType.Validator.isOptional(
        EvilType.Validator.isOr(isLayoutAngle, EvilType.Validator.isJust(0 as const))), anglePerDepth: EvilType.Validator.isOptional(EvilType.Validator.isJust(
        0 as const)), });
